    ...
    <div className="flex h-screen items-center justify-center">
      <div className="max-w-xl rounded p-8 shadow">
        <div className="mb-6 text-center w-full">
          ...
          <p className="text-muted-foreground mb-4">
            The Data Visualization is currently unavailable because not all
            delegates have completed their submissions. Please wait until all
            delegates have submitted their forms to view the dashboard.
          </p>
        </div>
        <table className="w-full table-auto border text-sm rounded">
          ...
          <tbody className="w-full">
            {(delegates ?? []).map((d, idx) => (
              <tr key={d.formSubmissionCode ?? idx} className="hover:bg-accent">
              ...
              <TabsContent key={sub.id} value={sub.id}>
                <Card key={sub.id}>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Progress
                      value={
                        sub.maxScore ? (sub.totalScore / sub.maxScore) * 100 : 0
                      }
                    />
                    <div>
                      <a
